...
cap = cv2.VideoCapture('../video/video_12.mp4')
...
while True:
    ...
    for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
        ...
        if matches[best_match]:
            ...
            frame = rectangle_frame(frame, left, top, right, bottom,
                                    rect_color=(255, 87, 51), corner_color=(255, 87, 51))
    ...
    resize = cv2.resize(frame, (1240, 680))

    cv2.imshow('Face Recognition', resize)

    cv2.imwrite("../output/face-recognition-video.jpg", resize)
